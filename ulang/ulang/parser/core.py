...
class AnnoFuncInsertPass(ast.NodeTransformer):
    ...
    __module__ = __name__
    __qualname__ = "AnnoFuncInsertPass"
...
class Parser:
    ...
    def parse(self, source, filename=""):
        ...
        try:
            ...
            try:
                raise SyntaxError(
                    message="unknown token is found here",
    ...
    def block(self, p):
        if len(p) == 3:
            if p[1]:
                return p[1]
        return [ast.Pass(lineno=(self.getlineno(p)), col_offset=(self.getcolno(p)))]
    ...
    def operator_setitem(self, p):
        if p[1] != "__getitem__":
            raise SyntaxError(
                message="param number mismatched for the operator",
                filename=(self.filename_),
        ...
        args.args.insert(
            0,
            ast.arg(
                arg="self", lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
            ),
        )
    ...
    def import_stmt(self, p):
        return ast.Import(
            names=(p[1]), lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def expr_stmt(self, p):
        ...
        return ast.Expr(
            value=(p[0]), lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def ret_stmt(self, p):
        ...
        return ast.Return(
            value=value, lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def assignment(self, p):
        ...
        return ast.Assign(
            [p[0]], (p[2]), lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def break_stmt(self, p):
        return ast.Break(lineno=(self.getlineno(p)), col_offset=(self.getcolno(p)))

    @pg_.production("continue_stmt : CONTINUE")
    def continue_stmt(self, p):
        return ast.Continue(lineno=(self.getlineno(p)), col_offset=(self.getcolno(p)))
    ...
    def range_expr_(self, p):
        ...
        if len(args) == 3:
            raise SyntaxError(
                message=('unexpected token "%s"' % p[1].getstr()),
                filename=(self.filename_),
                lineno=(self.getlineno(p[1])),
                colno=(self.getcolno(p[1])),
                source=(self.source_),
    ...
    def unary_expr(self, p):
        ...
        return ast.UnaryOp(
            operator, (p[1]), lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def slice_indeces(self, p):
        p[0].append(p[2])
        return ast.ExtSlice(
            dims=[self.slice_index([e]) for e in p[0]],
            lineno=(self.getlineno(p)),
            col_offset=(self.getcolno(p)),
        )
    ...
    def lambda_param(self, p):
        ...
        if isinstance(p[0], ast.Name):
            ...
            arg = ast.arg(
                arg=(p[0].id), lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
            )
    ...
    def strlit(self, p):
        ...
        if ret is not None:
            return ret
        return ast.Str(
            string, lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def parse_format_str(self, string, p):
        ...
        pos = 0
        while True:
            match = pattern.search(string=string, pos=pos)
            if match is None:
                break
            pos = match.span()[1]
            expr = match.group(1)
    ...
    def const_true(self, p):
        return ast.NameConstant(
            value=True, lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )

    @pg_.production("name_const : FALSE")
    def const_false(self, p):
        return ast.NameConstant(
            value=False, lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )

    @pg_.production("name_const : NIL")
    def const_nil(self, p):
        return ast.NameConstant(
            value=None, lineno=(self.getlineno(p)), col_offset=(self.getcolno(p))
        )
    ...
    def if_stmt(self, p):
        if len(p) == 0:
            ...
            p[-1] = isinstance(p[(-1)], list) or [p[(-1)]]
        return ast.If(
            test=(p[1]),
            body=(p[2]),
            orelse=(p[(-1)]),
    ...
    def error_handler(self, token):
        if token.getstr() == "\n":
            return
        raise SyntaxError(
            message=('unexpected token "%s"' % token.gettokentype()),
            filename=(self.filename_),
    ...
    def legalize_arguments(self, arguments):
        ...
        for arg in arguments.args:
            if hasattr(arg, "default") and arg.default:
                ...
                if hasDefaults:
                    raise SyntaxError(
                        message="no default expr is provided here",
                        filename=(self.filename_),
                        lineno=(arg.lineno),
...
# okay decompiling ./pyc/ulang.parser.core.pyc
