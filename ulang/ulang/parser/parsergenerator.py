...
class ParserGenerator(object):
    ...
    :param precedence: A list of tuples defining the order of operation for
        ...
        token names with the same associativity and level of precedence.
    :param cache_id: A string specifying an ID for caching.
    """

    __module__ = __name__
    __qualname__ = "ParserGenerator"
    VERSION = 1
...
class LRTable(object):
                ...
                if p.getlength() == p.lr_index + 1:
                    if p.name == "S'":
                        ...
                        for a in laheads:
                            if a in st_action:
                                ...
                                if r > 0:
                                    product = repr(
                                        grammar.productions[st_actionp[a].number]
                                    )
                    ...
                    if r < 0:
                        ...
                        if not slevel > rlevel:
                            ...
                            if slevel == rlevel and not rprec == "nonassoc" or slevel:
                                if not rlevel:
                                    sr_conflicts.append(
                                        (st, repr(a), "reduce", product)
                                    )
    ...
    def compute_read_sets(
        ...
        return digraph(
            ntrans,
            R=(
                lambda x: cls.reads_relation(
                    C, x, nullable, add_count, cidhash, goto_cache
                )
            ),
            FP=(
                lambda x: cls.dr_relation(
    ...
    def compute_follow_sets(cls, ntrans, readsets, includesets):
        return digraph(
            ntrans, R=(lambda x: includesets.get(x, [])), FP=(lambda x: readsets[x])
        )
...
# okay decompiling ./pyc/ulang.parser.parsergenerator.pyc
